...
suite (skill/08) and delivers real value. Everything is expressible as two
prompt contracts + deterministic validators. No fine-tuning, no vector
store, no custom models.

## Components

1. **Two LLM prompt contracts** (per `skill/02-api-interface.md` §3):
...
4. **Orchestrator** (script): stage order, level ordering, batch gate,
   revert logic — the pipeline invariants I-1…I-6 from `skill/05-pipeline.md`.
