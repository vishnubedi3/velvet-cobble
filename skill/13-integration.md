...
shims that implement `llm()`. The contracts in `skill/02-api-interface.md`
...
- **Context budgeting.** The knowledge load is large; use the retrieval
  strategy from `skill/12-advanced-architecture.md` §1 (per-finding
...
- If the host has disclosure requirements (venue policies, platform
  rules), wire the skill's `provenance` fields (`skill/03-input-schema.md`
...
1. Start with `skill/11-minimal-architecture.md` (two contracts + validators).
...
- [ ] Adversarial suite (skill/08) passes: A-T1…A-T23.
- [ ] Pipeline invariants I-1…I-6 hold (skill/05).
