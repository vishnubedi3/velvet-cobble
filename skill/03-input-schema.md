...
Machine-readable spec: `schemas/analysis-report.schema.json` is the output
side; this file is the normative human-readable input contract. (A
`skill-input.schema.json` is in `schemas/`.)
...
| `analysis_options` | object | no | thresholds overrides, level cap, batch gating flags (defaults in `skill/06-scoring.md`) |
