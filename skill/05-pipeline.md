...
1. Validate `SkillInput` (`skill/03-input-schema.md`).
...
3. **Pass C — evidence + intent.** Attach evidence/confidence/cause to each
   ...
   (`interventions/02` §3). Unsupported candidates → Observations.

## Stage 2 — Prioritize

Score findings (`skill/06-scoring.md`); sort; drop below-threshold items
...
5. After every level: re-run the **Level 0 re-check**
   (`interventions/01` §2.4) — previously preserved patterns may now be
...
Emit `SkillReport` (`skill/04-output-schema.md`): analysis, intervention log
