...
| `arena/<id>-velvet-cobble` (**Arena** / Arena Splash) | **Current working branch.** Session / development heads where the author is actively writing. Some merge to `main` via PR. At least one live session branch has **diverged materially**: different charter wording, optional root `AGENTS.md`, `samur/narrative/`, `samur/05-quality/`, `ops/`, skills moved under `skills/`. | Consult **aggressively** as current authoring state. **Not automatically established canon.** Inspect against `main` and **classify content**. Do not auto-merge. Do not ignore. Newer Arena does not override `main`. |
...
Do **not** implement "Arena is non-canon." Do **not** implement "Arena is canon because it is the working branch." Arena may confirm, clarify, extend, indicate intended direction, propose, explore, contradict, retcon, or be abandoned.
