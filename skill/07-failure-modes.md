...
- **Countermeasure.** Hard exclusion (`interventions/02` §7). Quality
...
- **Countermeasure.** Detector scores are never an input
  (`skill/06-scoring.md` §6). The skill's objective is explicit in SKILL.md
  §1.
...
- **Countermeasure.** The new-artifact rule (`interventions/02` §2):
...
- **Countermeasure.** Cause clustering (`skill/06-scoring.md` §3), Level 0
...
- **Countermeasure.** The objective is literary quality (SKILL.md §1); the
  benchmark's metrics (skill/09) measure function, not indistinguishability.
