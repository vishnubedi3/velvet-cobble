...
[Score]   priority model (skill/06) → queue
...
- Per-level contracts as in `interventions/01`; the LLM receives the
...
- Benchmark runner (skill/09 + benchmark/): seeded cases, traps, genre
...
- Audit log: full InterventionLog persisted per run (skill/04).
