...
skill as pure functions over data; `../spec/11-minimal-architecture.md`/`12` map the shapes onto
...
- Quality checks: every field cites a draft span; unknown fields marked
  ...
  recommends (a baseline that prescribes style is invalid).
...
- In: draft span + finding + StoryModel + level instructions
  (`../interventions/01-intervention-hierarchy.md`). Out: proposed edit(s) with a per-sentence rationale.
...
(`../taxonomy/17-genre-and-model-variation.md` §17.3) — but absence of metadata degrades gracefully
...
- All thresholds are expressed as *parameters* (defaults in
  `../spec/06-scoring.md`), recalibratable per analyzer model.
